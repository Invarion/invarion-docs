...
These rules apply when Codex is triggered from comments on RapidOnline release notes automation pull requests in this repository.

Automation PRs are docs-only PRs that update:

```text
docs/rapidplan-online/release-notes/release-notes-rapidplan-online.md
```

They use a branch name like:
...
- Sanitize release-note wording before publishing. Do not include internal IDs, source PR numbers, commit hashes, Notion links, or implementation details in the release notes file.
...
2. Fetch the latest release notes file from the existing PR head branch.
3. Apply the requested changes to the existing PR head branch.
4. Update the PR description candidate list when needed.
5. Fetch PR metadata again and verify the head SHA changed.
6. Fetch the release notes file from the new PR head SHA and verify the requested include/remove change is present in the target month section.
7. Post a PR comment summarizing applied and skipped commands, including the new commit SHA.
...
If the release notes automation PR body or linked runbook specifies a Slack follow-up channel, send the follow-up only after the GitHub publication verification succeeds. If Slack access cannot be verified, stop before committing when the runbook requires Slack verification.
