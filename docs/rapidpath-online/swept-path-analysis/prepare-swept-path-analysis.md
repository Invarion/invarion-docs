...
Finish drawing the path by clicking the right-click. Once the Vehicle Path is finished it turns into a single object that can be freely moved and rotated.
...
## Style the vehicle path, sweep envelope, and wheel paths
...
- show/hide directional arrows that indicate the start of each path section,
...
## Add a vehicle profile to the plan
