...
A cloud plan can also be created by clicking **New Cloud Plan**. This creates a default plan using the defaults you have set in **Tools** > **Preferences** > **Defaults**. Note that default plans do not have a location set initially and so won't have aerial mapping available. If necessary to add a location to a default plan: Click **F2** to open the *Edit Plan Properties* menu > uncheck the **Don't Set Location** box > choose your location.
