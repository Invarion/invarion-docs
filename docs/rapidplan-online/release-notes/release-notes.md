...
- [New feature](https://online.invarion.com/tips/22): User can select newly created object by using double right-click or double 2-finger tap.
...
[Autosave Notification Panel](/rapidplan-online/rapidplan-online-workspace/autosave-feature).
...
A Comments palette is shown in the side panel where comments can be placed. Viewers can also create a comment on the canvas by right-clicking. Comments are saved in the cloud and are available for all users who have access to the plan.
...
**Context menu** is now by default displayed in slimmer format below an object, allowing more of the canvas to be displayed.
...
Rich Text dialog now loads immediately when opened.
...
- Proper handling of right-click **control point**.
...
Easily create a QR code on your plan. Place it in the canvas and type in the URL/link you need the code to link to.
